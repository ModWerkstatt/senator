function data()
return {
	vehicles = {
		{ name = "vehicle/train/vt10.5/senator/kopf1_2.mdl", forward = true },
		{ name = "vehicle/train/vt10.5/senator/jakobs.mdl", forward = true },
        { name = "vehicle/waggon/vt10.5/senator_2/wagen1.mdl", forward = true },
        { name = "vehicle/train/vt10.5/senator/jakobs.mdl", forward = true },
        { name = "vehicle/waggon/vt10.5/senator_2/wagen2.mdl", forward = true },
        { name = "vehicle/train/vt10.5/senator/jakobs.mdl", forward = true },
        { name = "vehicle/waggon/vt10.5/senator_2/wagen3.mdl", forward = true },
        { name = "vehicle/train/vt10.5/senator/jakobs.mdl", forward = true },
        { name = "vehicle/waggon/vt10.5/senator_2/wagen1.mdl", forward = true },
        { name = "vehicle/train/vt10.5/senator/jakobs.mdl", forward = true },
        { name = "vehicle/waggon/vt10.5/senator_2/wagen5.mdl", forward = true },
        { name = "vehicle/train/vt10.5/senator/jakobs.mdl", forward = true },
        { name = "vehicle/train/vt10.5/senator/kopf2_2.mdl", forward = false },
	},
	name = _("senator_2_name"),
	desc = _("senator_desc"),
    groupFileName = "vehicle/train/vt10.5/menu_vt10.5.mdl",
}
end
